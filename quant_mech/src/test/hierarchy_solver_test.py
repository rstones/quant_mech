...
plt.subplot(121)
plt.plot(time[:-1], [dm[0,0] for dm in dm_history])
plt.plot(time[:-1], [dm[1,1] for dm in dm_history])
plt.subplot(122)
plt.plot(time[:-1], np.abs([dm[0,1] for dm in dm_history]))
plt.show()

# print 'Calculating steady state...'
# start = tutils.getTime()
# 
# ss = hs.calculate_steady_state(20,20)
# print ss
# print np.trace(ss)
# 
# end = tutils.getTime()
