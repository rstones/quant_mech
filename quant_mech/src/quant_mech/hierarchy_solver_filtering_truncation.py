...
def generate_hierarchy_couplings(num_dms, num_indices, truncation_level, dm_per_tier, coefficients):
    ...
    graph_elements = np.zeros((num_indices, num_non_zero_elements_per_idx), dtype=np.float64)
        ...
        for i in range(previous_level.shape[0]):
            ...
            for n in range(vec.size):
                ...
                for idx in range(vec.size):
                    coupling *= np.abs(coefficients[idx])**vec[idx] / np.prod(np.arange(1,vec[idx]+1))
                ...
                lower_coupling_elements[n, current_non_zero_element_idx] = np.sqrt(vec[n])
                ...
                higher_coupling_elements[n, current_non_zero_element_idx] = np.sqrt((vec[n]+1))
    ...
    # calculate ados to remove here
    ados_to_delete = np.zeros((num_dms,3), dtype=np.int16) # don't know num ados to remove yet but can't append to list for memory allocation reasons
    counter = 0
    for n in range(num_indices):
        for i in range(truncation_level-1):
            ...
            max_coupling = np.max(tier)
            for j,el in enumerate(tier):
                if el < max_coupling/100:
                    # length 3 list here are: idx of ado in n_vectors, row idx of coupling matrices, col idx of coupling matrices
                    ados_to_delete[counter] = [np.sum(dm_per_tier[:i])+j, graph_row_indices[n,np.sum(dm_per_tier[:i])+j],graph_col_indices[n,np.sum(dm_per_tier[:i])+j]]
                    counter += 1
    ados_to_delete = ados_to_delete[:counter] # can get rid of excess rows now
    
    return hierarchy, higher_coupling_elements, higher_coupling_row_indices, higher_coupling_column_indices, \
                            lower_coupling_elements, lower_coupling_row_indices, lower_coupling_column_indices, ados_to_delete
...
def generate_hierarchy_graph(num_dms, num_indices, truncation_level, dm_per_tier, coefficients):
    ...
    return hierarchy, graph_elements, graph_row_indices, graph_col_indices, ados_to_delete